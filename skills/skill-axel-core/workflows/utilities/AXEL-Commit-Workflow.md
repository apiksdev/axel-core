...
<document type="workflow">
  ...
  <objective>
    Smart git commit workflow that:
  ...
  <execution flow="linear">
    ...
    Step 1 - Route Action (Optional):
    - If param.action = "help":
      * Show usage information:
        ## Commit Workflow

        Usage: trigger="commit" with optional action parameter
    ...
    - If only unstaged files exist (no staged files):
      ...
      * If 2 → Show "Commit cancelled by user.", exit workflow
